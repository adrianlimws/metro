...
export function useRouteShapes() {
    ...
    const getShapeIdsForRoute = async (routeId) => {
        try {
            ...
            const tripsResponse = await fetch(
                `${API_CONFIG.CLOUDFLARE_API_BASE}/api/trips?route_id=${routeId}`
            )
            if (!tripsResponse.ok) {
            ...
            const shapeIds = [
                ...new Set(trips.map((trip) => trip.shape_id).filter(Boolean)),
            ]
            console.log(
                `Found ${shapeIds.length} shape IDs for route ${routeId}:`,
                shapeIds
            )
            ...
            if (realShapeIds.length > 0) {
                console.log(
                    `✅ Found ${realShapeIds.length} real GTFS shape IDs:`,
                    realShapeIds
                )
                return realShapeIds
            } else {
                console.log(
                    `⚠️ No real GTFS shape IDs found, using fallback approach`
                )
                ...
                console.log(
                    `🔄 Using correct real GTFS shapes for route ${routeNumber}:`,
                    fallback
                )
                return fallback
            }
        ...
        try {
            ...
            for (const shapeId of shapeIds) {
                const response = await fetch(
                    `${API_CONFIG.CLOUDFLARE_API_BASE}/api/shapes?shape_id=${shapeId}`
                )
                if (!response.ok) {
                ...
                console.log(
                    `✅ Loaded real GTFS shape ${shapeId} with ${shapePoints.length} points`
                )
            }
    ...
    const getRouteShapes = async (routeId) => {
        ...
        try {
            ...
            console.log(
                `✅ Successfully loaded ${shapesData.length} real GTFS shapes for route ${routeId}`
            )
            ...
            if (routesResponse.ok) {
                ...
                if (route && route.route_color) {
                    ...
                    const color = route.route_color.startsWith('#')
                        ...
                        : `#${route.route_color}`
                    console.log(
                        `Using route color ${color} for route ${routeId}`
                    )
                    return color
                }
            }
